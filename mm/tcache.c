...
tcache_invalidate_node_tree(struct tcache_node_tree *tree)
{
	...
	while (!RB_EMPTY_ROOT(&tree->root)) {
		...
		WARN_ON(refcount_read(&node->kref.refcount) != 1);
...
repeat:
	...
	if (pagep) {
		...
		if (!folio_try_get_rcu((struct folio *)page))
			goto repeat;
		/*
		 * Has the page moved?
		     ...
		     struct page **pages, int nr_to_scan)
{
	...
	while (nr_to_scan-- > 0 && !list_empty(&pni->lru)) {
		...
		if (unlikely(!folio_try_get_rcu((struct folio *)page)))
			continue;
...
static int tcache_cleancache_init_shared_fs(uuid_t *uuid, size_t pagesize)
{
	return -1;
}
...
static int __init tcache_init(void)
{
	...
	err = register_shrinker(&tcache_shrinker, "tcache-shrinker");
	if (err)
		goto out_free_lru;

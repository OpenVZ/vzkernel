...
void clear_shrinker_bit(struct mem_cgroup *memcg, int nid, int shrinker_id)
{
	struct shrinker_info *info;

	/*
	 * The info for refcounted memcg can only be freed in
	 * free_shrinker_info so we can safely protect
	 * map with rcu_read_lock.
	 */
	rcu_read_lock();
	info = rcu_dereference(memcg->nodeinfo[nid]->shrinker_info);
	clear_bit(shrinker_id, info->map);
	rcu_read_unlock();
}
...
static int shrink_one(struct lruvec *lruvec, struct scan_control *sc)
{
	...
	shrink_slab(sc->gfp_mask, pgdat->node_id, memcg, sc->priority, false);

		      ...
		      const char *message)
{
	...
	rcu_read_lock();
	pr_err("Killed process %d (%s), UID %d in VE \"%s\" total-vm:%lukB, anon-rss:%lukB, file-rss:%lukB, shmem-rss:%lukB\n",
		task_pid_nr(victim), victim->comm, task_uid(victim).val,
		task_ve_name(victim), K(victim->mm->total_vm),

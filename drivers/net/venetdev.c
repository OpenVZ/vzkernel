...
static int venet_get_link_ksettings(struct net_device *dev,
...
static const struct ethtool_ops venet_ethtool_ops = {
	.get_link_ksettings	= venet_get_link_ksettings,
...
static void venet_setup(struct net_device *dev)
{
	...
	dev->priv_destructor = venet_destructor;
...
static int venet_newlink(struct net *src_net,
...
static struct proc_ops proc_veip_operations = {
	.proc_open	= veip_open,
	.proc_read	= seq_read,
...
static ssize_t ve_ip_access_write(struct kernfs_open_file *of,
				  char *buf, size_t bytes, loff_t off)
{
	struct ve_struct *ve = css_to_ve(of_css(of));
	const struct cftype *cft = of_cft(of);
	struct ve_addr_struct addr;
	int ret;
	...
	if (strncmp(cft->name, "ip6", 3)) {
		if ((ret = in4_to_veaddr(buf, &addr)) != 0)
			return ret;
	} else {
		if ((ret = in6_to_veaddr(buf, &addr)) != 0)
			return ret;
	}

	ret = do_ve_ip_map(ve, cft->private, &addr);
	if (ret)
		return ret;
...
static int ve_ip_access_seq_read(struct seq_file *seq, void *v)
{
	struct ve_struct *ve = css_to_ve(seq_css(seq));
	struct ip_entry_struct *s;
	char buf[40];
	int family = strncmp(seq_cft(seq)->name, "ip6", 3) ? AF_INET : AF_INET6;
	int i;
...
static struct cftype venet_cftypes[] = {
	...
	{
		.name = "ip_list",
		.seq_show = ve_ip_access_seq_read,
		.flags = CFTYPE_NOT_ON_ROOT,
	},
	{
		.name = "ip6_allow",
...
static int venet_changelink(struct net_device *dev,
			    struct nlattr *tb[],
			    struct nlattr *data[],
			    struct netlink_ext_ack *extack)
{
...
__init int venet_init(void)
{
	...
	err = cgroup_add_legacy_cftypes(&ve_cgrp_subsys, venet_cftypes);
	if (err)
		goto err_proc;
...
{
	cgroup_rm_cftypes(venet_cftypes);

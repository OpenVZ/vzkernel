...
static inline int broadcast_write(struct phy_device *phydev, u32 regnum,
				  u16 val)
{
	return mdiobus_write(phydev->mdio_bus, BROADCAST_ADDR, regnum, val);
}
			   ...
			   struct phy_device *phydev)
{
	...
	if (chosen_phy == phydev->mdio_addr)
		return 1;
...
static int dp83640_probe(struct phy_device *phydev)
{
	...
	if (phydev->mdio_addr == BROADCAST_ADDR)
		return 0;

	clock = dp83640_clock_get_bus(phydev->mdio_bus);
	if (!clock)
		goto no_clock;

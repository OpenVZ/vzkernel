...
static int queue_supports_no_sg_merge(struct dm_target *ti, struct dm_dev *dev,
				      sector_t start, sector_t len, void *data)
{
	struct request_queue *q = bdev_get_queue(dev->bdev);
			       ...
			       struct queue_limits *limits)
{
	...
	/* Set QUEUE_FLAG_NO_SG_MERGE if any underlying queue enables it */
	if (dm_table_any_dev_attr(t, queue_supports_no_sg_merge))

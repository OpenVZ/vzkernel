				...
				char *result, unsigned int maxlen)
{
	...
	spin_lock_irq(&ploop->pb_lock);
	ret = -ENOENT;
	h = list_first_entry_or_null(&pb->pending, typeof(*h), list);
	if (!h)
		goto unlock;
	list_move_tail(&h->list, &pb->reported);
	DMEMIT("%u", h->cluster);
	ret = 1;
unlock:
	spin_unlock_irq(&ploop->pb_lock);
		  ...
		  char *result, unsigned int maxlen)
{
	...
	if (!strcmp(argv[0], "resize")) {
		...
		if (argc != 3 || kstrtou64(argv[2], 10, &val) < 0)
			goto unlock;
		ret = ploop_push_backup_write(ploop, argv[1], val);
	} else {
		ret = -ENOTSUPP;
	}

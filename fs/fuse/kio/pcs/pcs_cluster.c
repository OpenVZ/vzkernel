...
static void fiemap_get_iter(void * datasource, unsigned int offset, struct iov_iter *it, unsigned int direction)
{
	struct fiemap_iterator * iter = datasource;

	BUG_ON(offset >= PCS_FIEMAP_BUFSIZE);
	iter->buffer_kv.iov_base = iter->buffer;
	iter->buffer_kv.iov_len = PCS_FIEMAP_BUFSIZE;
	iov_iter_kvec(it, direction, &iter->buffer_kv, 1, PCS_FIEMAP_BUFSIZE);
	...
	if (iter->fiemap_max == 0) {
		*iter->mapped += count;
		...
		for (i = 0; i < count; i++) {
			...
			len = copy_to_iter(&e, sizeof(e), &it);
			BUG_ON(len != sizeof(e));
			(*iter->mapped)++;
		}
	}

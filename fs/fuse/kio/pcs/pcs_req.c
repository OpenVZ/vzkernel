...
static void ireq_timer_handler(struct timer_list *timer)
{
	struct pcs_int_request *ireq = container_of(timer, struct pcs_int_request, timer);
...
void ireq_handle_hole(struct pcs_int_request *ireq)
{
	...
	while (len > 0) {
		...
		copy = iov_iter_zero(min(iov_iter_count(&it), (size_t)len), &it);

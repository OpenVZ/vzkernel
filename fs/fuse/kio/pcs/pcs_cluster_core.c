...
static int pcs_mapset_init(struct pcs_cluster_core *cc)
{
	struct pcs_map_set *maps = &cc->maps;
	dev_t dev;
	...
	/* cc.fc is not yet initialized here, but pfc->fc is filled already */
	dev = container_of(cc, struct pcs_fuse_cluster, cc)->fc->dev;
	register_shrinker(&maps->shrinker, "pcs-maps:%u", dev);

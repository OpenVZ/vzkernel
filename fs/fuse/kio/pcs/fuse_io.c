...
static void req_fiemap_get_iter(void *data, unsigned int offset, struct iov_iter *it, unsigned int direction)
{
	struct pcs_fuse_req * r = data;
	struct pcs_int_request *ireq = &r->exec.ireq;

	if (offset < sizeof(struct fiemap)) {
		r->exec.io.kv.iov_base = r->req.args->out_args[1].value;
		r->exec.io.kv.iov_len = sizeof(struct fiemap);
		iov_iter_kvec(it, direction, &r->exec.io.kv, 1, sizeof(struct fiemap));
	} else {
		offset -= sizeof(struct fiemap);
		iov_iter_bvec(it, direction, r->exec.io.bvec, r->exec.io.num_bvecs,
				   ireq->apireq.aux*sizeof(struct fiemap_extent));
	}
	iov_iter_advance(it, offset);
}

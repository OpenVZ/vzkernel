...
static void req_get_iter(void *data, unsigned int offset, struct iov_iter *it)
{
	...
	iov_iter_bvec(it, ITER_BVEC, r->exec.io.bvec, r->exec.io.num_bvecs, r->exec.io.req.size);
...
static void req_fiemap_get_iter(void *data, unsigned int offset, struct iov_iter *it)
{
	...
	if (offset < sizeof(struct fiemap)) {
		r->exec.io.kv.iov_base = r->req.out.args[1].value;
		r->exec.io.kv.iov_len = sizeof(struct fiemap);
		iov_iter_kvec(it, ITER_KVEC, &r->exec.io.kv, 1, sizeof(struct fiemap));
	} else {
		offset -= sizeof(struct fiemap);
		iov_iter_bvec(it, ITER_BVEC, r->exec.io.bvec, r->exec.io.num_bvecs,
		       ...
		       void (*complete)(struct _pcs_api_iorequest_t *))
{
	...
	{
	case PCS_REQ_T_READ:
		set_io_buff(r, offset, size, r->req.out.page_zeroing);
		break;

...
void fuse_release_common(struct file *file, bool isdir)
{
	 ...
	 */
	fuse_file_put(ff, ff->fc->destroy || ff->fc->close_wait, isdir);
}
			      ...
			      fl_owner_t owner)
{
	...
	return fuse_simple_check_request(fc, &ia->ap.args, ff);
}
...
fuse_direct_IO(struct kiocb *iocb, struct iov_iter *iter)
{
	 ...
	 */
	io->async = ff->fc->async_dio | ff->fc->writeback_cache;
	io->iocb = iocb;
			       ...
			       __u64 * start_p, __u64 * len_p, int * last_p, struct fiemap_extent_info * dest)
{
	...
	ap.args.in_numargs = 2;
	ap.args.in_args[0].size = sizeof(inarg);
	ap.args.in_args[0].value = &inarg;
	ap.args.in_args[1].size = sizeof(ifiemap);
	ap.args.in_args[1].value = &ifiemap;

	ap.args.out_numargs = npages ? 3 : 2;
	ap.args.out_args[0].size = sizeof(outarg);
	ap.args.out_args[0].value = &outarg;
	ap.args.out_args[1].size = sizeof(ofiemap);
	ap.args.out_args[1].value = &ofiemap;
	if (npages) {
		ap.args.out_args[2].size = npages*PAGE_SIZE;
		...
		for (allocated = 0; allocated < npages; allocated++) {
			ap.pages[allocated] = alloc_page(GFP_KERNEL | __GFP_HIGHMEM);
			if (!ap.pages[allocated])
				goto out;
			ap.descs[allocated].length = PAGE_SIZE;
		}
	}
		...
		__u64 start, __u64 len)
{
	...
	for (;;) {
		...
		npages = (cur_max*sizeof(struct fiemap_extent) + PAGE_SIZE - 1) / PAGE_SIZE;
		if (npages > FUSE_DEFAULT_MAX_PAGES_PER_REQ) {
			npages = FUSE_DEFAULT_MAX_PAGES_PER_REQ;

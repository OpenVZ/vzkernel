...
#define XATTR_VE_PREFIX		XATTR_SYSTEM_PREFIX "ve_access."

static size_t
sysfs_ve_access_list(struct dentry *dentry, char *list, size_t list_size,
		const char *name, size_t name_len, int type)
{
	...
	/* Hide xattrs from containers */
	if (!ve_is_super(get_exec_env()))
		return 0;

	ret = inode_permission(dentry->d_inode, MAY_READ);
	if (ret)
		return ret;
	...
	hlist_for_each_entry_rcu(link, pos, &sd->s_ve_perms->links, map_link) {
		...
		ret += strlcpy(list + ret, ve_name(ve),
				list_size > ret ? list_size - ret : 0) + 1;
	}
	rcu_read_unlock();
	return ret;
}
...
static int
sysfs_ve_access_get(struct dentry *dentry, const char *name, void *buffer,
		     size_t size, int type)
{
	...
	rcu_read_lock();
	if (ve) {
		link = kmapset_lookup(sd->s_ve_perms, &ve->ve_sysfs_perms);
		if (link)
			mask = link->value;
		else
			ret = -ENODATA;
...
static int
sysfs_ve_access_set(struct dentry *dentry, const char *name, const void *value,
		     size_t size, int flags, int type)
{
	struct sysfs_dirent *sd = dentry->d_fsdata;
	struct kmapset_map *map, *old_map;
	struct ve_struct *ve;
	char mask;
	int ret;
	...
	ret = inode_permission(dentry->d_inode, MAY_WRITE);
	if (ret)
		return ret;
	...
	mutex_lock(&sysfs_mutex);
	old_map = kmapset_get(sd->s_ve_perms);
	mutex_unlock(&sysfs_mutex);
	map = kmapset_dup(old_map);
	kmapset_put(old_map);
	ret = -ENOMEM;
	if (!map)
		goto out_copy;
...
static const struct xattr_handler sysfs_ve_access_handler = {
	.prefix = XATTR_VE_PREFIX,
	.list	= sysfs_ve_access_list,
	.get	= sysfs_ve_access_get,
	.set	= sysfs_ve_access_set,
};

#endif /* CONFIG_VE */

const struct xattr_handler *sysfs_xattr_handlers[] = {
#ifdef CONFIG_VE
	&sysfs_ve_access_handler,
#endif
	NULL
};
...
{
	return generic_listxattr(dentry, buffer, buffer_size);
}
		...
		size_t size, int flags)
{
	...
	if (!strncmp(name, XATTR_SECURITY_PREFIX, XATTR_SECURITY_PREFIX_LEN)) {
		...
		error = generic_setxattr(dentry, name, value, size, flags);
	} else
		return -EINVAL;
out:
	return error;

...
static void bt_clear_tag(struct blk_mq_bitmap_tags *bt, unsigned int tag)
{
	...
	if (wait_cnt <= 0) {
		unsigned int wake_batch = READ_ONCE(bt->wake_cnt);
		/*
			    ...
			    unsigned int depth)
{
	...
	if (wake_cnt > depth / BT_WAIT_QUEUES)
		wake_cnt = max(1U, depth / BT_WAIT_QUEUES);
	if (bt->wake_cnt != wake_cnt) {
		WRITE_ONCE(bt->wake_cnt, wake_cnt);
		/*
		 * Pairs with the memory barrier in bt_clear_tag() to ensure that

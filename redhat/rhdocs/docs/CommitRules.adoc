...
If the changeset has dependencies on other changesets, the MR description must include a mention of those changesets or their bugzillas.  Use one 'Depends:' line for each dependent changeset or bugzilla. +++<u>+++Follow the above 'Bugzilla:' guidelines for ‘Depends:’ lines referencing bugzillas (ie. Bugzillas must be specified as URLS and not IDs)+++</u>+++. Alternatively, you can include a full URL referencing another merge request in the same project to specify your dependencies, and the requisite bugzilla numbers will be extracted from the dependent changeset's description. Missing dependencies and malformed ‘Depends:’ lines will be identified by the project’s webhooks.
